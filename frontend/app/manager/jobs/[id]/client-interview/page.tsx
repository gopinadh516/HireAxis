...
import {
  ...
  CheckCircle2Icon, XCircleIcon, PauseCircleIcon,
...
function ClientInterviewCard({ jobId, jt, interview, onUpdated }: { jobId: string; jt: JobTalent; interview: Interview | undefined; onUpdated: () => void; }) {
  const t = jt.talents;
  ...
  async function handleSchedule() {
    ...
    try {
      ...
      if (interview) {
        ...
        await apiFetch(`/api/jobs/${jobId}/interviews`, { method: "POST", body: JSON.stringify({ talent_id: jt.talent_id, type: "client", ...payload }) });
      }
      toast.success("Client interview scheduled"); onUpdated();
    } catch (e) { toast.error(e instanceof Error ? e.message : "Failed"); }
    finally { setSaving(false); }
  }

  async function handleOutcome(outcome: "pass" | "fail" | "hold") {
    if (!interview) { toast.error("Schedule the interview first"); return; }
    setRecording(true);
    try {
      ...
      toast.success(`Client outcome: ${outcome}`); onUpdated();
    } catch (e) { toast.error(e instanceof Error ? e.message : "Failed"); }
  ...
  return (
    <div className={`rounded-xl border bg-card p-5 space-y-4 ${outcomeVal !== "pending" ? "border-border opacity-80" : "border-primary/30"}`}>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1"><Label className="text-xs">Client Interviewers</Label><Input value={interviewers} onChange={(e) => setInterviewers(e.target.value)} placeholder="Client contact names" className="h-8 text-xs" /></div>
      </div>
      ...
      {interview && outcomeVal === "pending" && (
        <div className="space-y-2 border-t border-border pt-4">
          ...
          <Textarea value={feedback} onChange={(e) => setFeedback(e.target.value)} placeholder="Client feedback notes…" className="min-h-16 text-xs resize-none" />
...
export default function ClientInterviewPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const loadData = useCallback(async () => {
    const [j, jt, iv] = await Promise.all([
      ...
      apiFetch<JobTalent[]>(`/api/jobs/${id}/job-talents?status=internal_passed`),
      apiFetch<Interview[]>(`/api/jobs/${id}/interviews?type=client`),
    ]);
  ...
  const passed = interviews.some((i) => i.outcome === "pass");

  async function handleAdvance() {
    setAdvancing(true);
    try {
      await apiFetch(`/api/jobs/${id}/pipeline-stage`, { method: "PATCH", body: JSON.stringify({ stage: "onboard" }) });
      toast.success("Advanced to Onboard");
      router.push(`/manager/jobs/${id}/onboard`);
    } catch (e) { toast.error(e instanceof Error ? e.message : "Cannot advance yet"); }
    finally { setAdvancing(false); }
  }

  if (loading) return <Shell role="manager" pageTitle="Client Interview"><div className="space-y-4">{[1,2].map((i) => <Skeleton key={i} className="h-40 w-full rounded-xl" />)}</div></Shell>;
  ...
  return (
    <>
      <Shell role="manager" pageTitle={job.title} pageSubtitle={formatJobId(job)}>
        <div className="mb-4">
          <Link href={`/manager/jobs/${id}/internal-interview`}>
            <Button variant="ghost" size="sm" className="h-7 gap-1.5 text-xs -ml-2"><ArrowLeftIcon className="size-3.5" /> Back to Internal Interview</Button>
          </Link>
        </div>

        <PipelineBar jobId={id} currentStage={job.pipeline_stage ?? "client_interview"} />
        ...
        <div className="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-3">
          ...
          {/* ── Right: client interview working area ── */}
          <div className="lg:col-span-2 space-y-4">
            <div>
              <h2 className="text-sm font-semibold">Client Interviews</h2>
              <p className="text-xs text-muted-foreground mt-0.5">Schedule and record client feedback for candidates who passed internal screening.</p>
            </div>

            {jobTalents.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border py-12 text-center">
                <p className="text-sm text-muted-foreground">No candidates passed internal interviews yet.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {jobTalents.map((jt) => (
                  <ClientInterviewCard key={jt.id} jobId={id} jt={jt} interview={interviews.find((i) => i.talent_id === jt.talent_id)} onUpdated={loadData} />
                ))}
              </div>
            )}

            <div className="flex justify-end pt-2">
              <Button onClick={handleAdvance} disabled={advancing || !passed} className="gap-1.5">
                {advancing ? "Checking…" : "Advance to Onboard"}<ChevronRightIcon className="size-4" />
              </Button>
            </div>
          </div>
        </div>
